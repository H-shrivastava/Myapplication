...
//    if (mediaView!= null && templateType==2){
...
//    if (template == R.layout.gnt_medium_template_view) {
//      return MEDIUM_TEMPLATE;
//    }  else if (template == R.layout.gnt_small_template_view) {
...
//  }
  ...
  private void initView(Context context, AttributeSet attributeSet) {
    ...
    try {
      template = attributes.getResourceId(
              R.styleable.TemplateView_gnt_template, R.layout.gnt_medium_template_view);
//      templateType = attributes.getInt(R.styleable.TemplateView_gnt_template_type, 0);
    } finally {
      attributes.recycle();
    }
    LayoutInflater inflater =
        (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    inflater.inflate(template, this);
  }
